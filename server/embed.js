...
export async function embed(inputs) {
    ...
    try {
        ...
        throw new EmbeddingError(`Cannot reach Ollama at ${OLLAMA_HOST}. Is it running? Start it with "ollama serve".`, { cause });
    }
    if (!res.ok) {
...
export async function embedOne(input) {
    const [vec] = await embed([input]);
    if (!vec)
        throw new EmbeddingError('Embedding returned no vector.');
    return vec;
}
/**
