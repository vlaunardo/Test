...
async function handleChat(req) {
    ...
    if (countChunks(db) === 0) {
        return Response.json({ error: 'The knowledge base is empty. Add markdown to content/ and run "bun run ingest".' }, { status: 409 });
    }
    const encoder = new TextEncoder();
    const stream = new ReadableStream({
        async start(controller) {
            const send = (e) => controller.enqueue(encoder.encode(sse(e)));
            try {
