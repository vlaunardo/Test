import type { Citation, ScoredChunk } from '../shared/types.ts'
 ...
 */
export function buildRetrievalQuery(question: string, history: { role: string; content: string }[]): string {
    const recentUser = history
        .filter((m) => m.role === 'user')
        .slice(-2)
        .map((m) => m.content)
    if (recentUser.length === 0 || question.length > 80) return question
