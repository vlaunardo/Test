...
if (tags) {
    ...
    if (names.some((n) => n.startsWith(EMBED_MODEL)))
        ok(`embedding model "${EMBED_MODEL}" installed`);
    else
        bad(`embedding model "${EMBED_MODEL}" missing`, `ollama pull ${EMBED_MODEL}`);
    if (PROVIDER === 'ollama') {
        const chatModel = process.env.OLLAMA_CHAT_MODEL || 'qwen3:4b';
        if (names.some((n) => n.startsWith(chatModel)))
            ok(`chat model "${chatModel}" installed`);
        else
            bad(`chat model "${chatModel}" missing`, `ollama pull ${chatModel}`);
    }
}
...
if (!needed)
    ok('no API key required (fully local)');
else if (process.env[needed])
    ok(`${needed} is set`);
else {
    const where = PROVIDER === 'gemini' ? 'get a free key at https://aistudio.google.com/apikey' : `set ${needed} in .env`;
...
try {
    ...
    if (n > 0)
        ...
        bad('index is empty', 'add markdown to content/ then run "bun run ingest"');
}
catch (err) {
